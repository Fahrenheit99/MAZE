...
//_________________RESOLUCIÓN DE PROBLEMAS[SUBRUTINAS]_________________
...
void print_mosue_tracker(){
  for (size_t i = 0; i < 2048; i++) {
    for (size_t j = 0; j < 2048; j++) {
      if(track_mouse[i][j])printf("track_mouse[%zu][%zu]\t|%d\n", i, j, track_mouse[i][j]);
    }
  }
}
...
void draw_wallsv2(cairo_t *cr, float xc, float yc, int ic, int jc){
  ...
  int local_wall[4];
      ...
      } else if(track_mouse[ic][jc]){
        //printf("track_mouse flag %d\n", track_mouse[ic][jc]);
        cairo_set_source_rgb(cr, 0, 0, .4);
...
//----------------------------------------------------
void  solver_module(cairo_t *cr, int i, int j){

      cairo_set_source_rgb(cr, 0, 0, 0.7);
      cairo_arc(cr, i+w_dimention/2, j+w_dimention/2, w_dimention/10, 0, 2 * 3.141592);
      cairo_stroke_preserve(cr);
      cairo_fill(cr);
...
static void do_drawing(cairo_t *cr){
  ...
  //cairo_scale(cr, scaleFactor/100, scaleFactor/100);
